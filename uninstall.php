...
/*
 * manages EasyMultilingual uninstallation
 * the goal is to remove ALL EasyMultilingual related data in db
 *
 * @since 0.5
 */
class EML_Uninstall {
	...
	function uninstall() {
		...
		// delete the strings translations 1.2+
		register_post_type('easyMultilingual_mo', array('rewrite' => false, 'query_var' => false));
		$ids = get_posts(array(
			'post_type'   => 'easyMultilingual_mo',
			'numberposts' => -1,
			'nopaging'    => true,
			'fields'      => 'ids',
		));
		...
		// delete options
		delete_option('easyMultilingual');
		delete_option('widget_easyMultilingual'); // automatically created by WP
