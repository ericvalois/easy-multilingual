...
Version: 1.5.4
Author: Bulledev
Original author: Frédéric Demarle (Polylang - http://polylang.wordpress.com/)
...
if (file_exists(EML_LOCAL_DIR . '/eml-config.php'))
	include_once(EML_LOCAL_DIR . '/eml-config.php');
...
if (!defined('EML_COOKIE'))
	define('EML_COOKIE', 'eml_language');
...
class EasyMultilingual {
	...
	public function __construct() {
		...
		if (!defined('EML_AJAX_ON_FRONT')) {
			...
			define('EML_AJAX_ON_FRONT', defined('DOING_AJAX') && DOING_AJAX && empty($_REQUEST['eml_ajax_backend']) && !$in);
		}
	...
	public function init() {
		...
		$class = apply_filters('eml_model', EML_SETTINGS ? 'EML_Admin_Model' : 'EML_Model');
