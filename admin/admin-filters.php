 ...
 * @since 1.2
 */
class EML_Admin_Filters extends EML_Filters {
	...
	public function in_widget_form($widget, $return, $instance) {
		...
		printf('<p><label for="%1$s">%2$s %3$s</label></p>',
			...
			__('The widget is displayed for:', 'easyMultilingual'),
			$dropdown->walk(
				array_merge(
					array((object) array('slug' => 0, 'name' => __('All languages', 'easyMultilingual'))),
	...
	public function personal_options($profileuser) {
		...
		printf('
			...
			__('Admin language', 'easyMultilingual'),
			$dropdown->walk(
				array_merge(
					array((object) array('locale' => 0, 'name' => __('Wordpress default', 'easyMultilingual'))),
					$this->model->get_languages_list()
				),
				array(
					'name'        => 'user_lang',
