...
class EML_Admin_Filters_Term {
	...
	public function __construct(&$easyMultilingual) {
		...
		$this->pref_lang = &$easyMultilingual->pref_lang;
		...
		add_action('edit_term', array(&$this, 'save_term'), 999, 3); // late as it may conflict with other plugins, see http://wordpress.org/support/topic/easyMultilingual-and-wordpress-seo-by-yoast
		...
		version_compare($GLOBALS['wp_version'], '3.7', '<') ?
			add_action('wp_ajax_easyMultilingual-ajax-tag-search', array(&$this,'ajax_tag_search')) :
			add_action('wp_ajax_easyMultilingual-ajax-tag-search', 'wp_ajax_ajax_tag_search'); // take profit of new filter, cache...
	...
	public function add_term_form() {
		...
		printf('
			...
			__('Sets the language', 'easyMultilingual')
		);
		...
		if ($lang)
			include(EML_ADMIN_INC.'/view-translations-term.php');
		echo '</div>'."\n";
	}
	...
	public function pre_insert_term($term, $taxonomy) {
		if (isset($_POST['action'], $_POST['from_tag'], $_POST['term_lang_choice']) && 'add-tag' == $_POST['action'] && $this->model->get_translation('term', $_POST['from_tag'], $_POST['term_lang_choice'])) {
			...
			return new WP_Error('term_translation_exists', sprintf(
				__('A translation does already exist for %s', 'easyMultilingual'),
				$from_term->name
			));
		}
		return $term;
	...
	public function terms_clauses($clauses, $taxonomies, $args) {
		...
		elseif (!empty($_GET['lang']) && isset($_GET['action']) && 'easyMultilingual-ajax-tag-search' == $_GET['action'])
