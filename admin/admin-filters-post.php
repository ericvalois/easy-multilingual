 ...
 * @since 1.2
 */
class EML_Admin_Filters_Post extends EML_Admin_Filters_Post_Base {
	 ...
	 * @since 1.2
	 *
	 * @param object $easyMultilingual
	 */
	public function __construct(&$easyMultilingual) {
		parent::__construct($easyMultilingual);
		$this->options = &$easyMultilingual->options;
		$this->curlang = &$easyMultilingual->curlang;
	...
	public function add_meta_boxes($post_type) {
		if ($this->model->is_translated_post_type($post_type))
			add_meta_box('ml_box', __('Languages','easyMultilingual'), array(&$this, 'post_language'), $post_type, 'side', 'high');
	}
	...
	public function post_language() {
		...
		if ($lang)
			include(EML_ADMIN_INC.'/view-translations-'.($post_type == 'attachment' ? 'media' : 'post').'.php');
		echo '</div>'."\n";
	}
    
    public function createTranslationTabs($post_id, $current_lang) {
            ...
            <h2 class="nav-tab-wrapper" style="padding-bottom:0px;">    
                ...
                <?php 
                    ...
                    foreach($this->model->get_languages_list() as $language) {
                        ...
                        $isCurrentLang = ($language->term_id == $current_lang->term_id);        
                        
                        // Pour le bon lien
                        $value = $this->model->get_translation('post', $post_id, $language);                        
                        if (!$value || $value == $post_id) $value = ''; // $value == $post_ID happens if the post has been (auto)saved before changing the language                            
                        if (isset($_GET['from_post'])) $value = $this->model->get_post((int)$_GET['from_post'], $language);

                        $link = esc_url($this->links->get_new_post_translation_link($post_id, $language));                        
                        if ($value) {
                            $selected = get_post($value);
                            $link = esc_url(get_edit_post_link($value));
                        }
                        ///////////
                                                
                        if ($language->term_id == $current_lang->term_id) {
                            ?>
                                <a class="nav-tab nav-tab-active" href="javascritp:;">
                                    <?php echo $language->flag ? $language->flag : esc_html($language->slug); ?> <?php echo $language->name; ?>
                                </a>
                                <input type="hidden" name="<?php echo $post_type == 'attachment' ? sprintf('attachments[%d][language]', $post_id) : 'post_lang_choice'; ?>" value="<?php echo esc_html($language->slug); ?>" />
                            <?php
                        }
                        else {
                            ?>
                                <a class="nav-tab " href="<?php echo $link; ?>">
                                    <?php echo $language->flag ? $language->flag : esc_html($language->slug); ?> <?php echo $language->name; ?>
                                </a>                                
                            <?php
                            printf('
                                <input type="hidden" name="post_tr_lang[%1$s]" id="htr_lang_%1$s" value="%2$s"/>
                                <input type="hidden" class="tr_lang" id="tr_lang_%1$s" value="%3$s">',
        ...
        <script type="text/javascript">
            jQuery(document).ready( function() {
                var postbox = jQuery("#eml-tabs").closest('.postbox');
                var eml_tabs_view = jQuery("#eml-tabs");
                jQuery("#poststuff").prepend(eml_tabs_view);
                postbox.remove();
            });            
        </script>
        <?php
	...
	public function post_lang_choice() {
		...
		ob_start();
		if ($lang)
			include(EML_ADMIN_INC.'/view-translations-post.php');
	 ...
	 * http://wordpress.org/support/topic/plugin-easyMultilingual-quick-edit-still-breaks-translation-linking-of-pages-in-072
	 *
	 * @since 0.1
	 *
	 * @param int $post_id
