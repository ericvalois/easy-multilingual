 ...
 * @since 1.2
 */
class EML_WP_Import extends WP_Import {
	...
	function process_terms() {
		...
		$easyMultilingual->model->clean_languages_cache(); // to update the languages list if needed

		if (($options = get_option('easyMultilingual')) && empty($options['default_lang']) && ($languages = $easyMultilingual->model->get_languages_list())) {
	...
	function process_posts() {
		...
		foreach ($this->posts as $post) {
			...
			if (0 === strpos($post['post_title'], 'easyMultilingual_mo_'))
				$mo_posts[] = $post;
		}

		if (!empty($mo_posts))
			new EML_MO(); // just to register the easyMultilingual_mo post type before processing posts

		parent::process_posts();

		global $easyMultilingual;
		$easyMultilingual->model->clean_languages_cache(); // to update the posts count in (cached) languages list
	...
	function remap_terms_relations(&$terms){
		global $easyMultilingual, $wpdb;

		foreach ($terms as $term) {
			$translations = unserialize($term['term_description']);
			foreach($translations as $slug => $old_id)
				if ($old_id && !empty($this->processed_terms[$old_id]) && $lang = $easyMultilingual->model->get_language($slug)) {
