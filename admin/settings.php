...
/*
 * a class for the EasyMultilingual settings pages
 *
 * @since 1.2
 */
class EML_Settings {
	...
	public function __construct(&$easyMultilingual) {
		$this->links_model = &$easyMultilingual->links_model;
		$this->model = &$easyMultilingual->model;
	...
	public function languages_page() {
		...
		$tabs = array('lang' => __('Languages','easyMultilingual'));

		// only if at least one language has been created
		if ($listlanguages = $this->model->get_languages_list()) {
			$tabs['strings'] = __('Strings translation','easyMultilingual');
		...
		switch($this->active_tab) {
			case 'lang':
				...
				$list_table = new EML_Table_Languages();
			...
			case 'strings':
				...
				$string_table = new EML_Table_String($groups, $selected);
		...
		switch ($action) {
			case 'add':
				...
				if ($this->model->add_language($_POST))
					EML_Admin::download_mo($_POST['locale']);
		...
		// displays the page
		include(EML_ADMIN_INC.'/view-languages.php');
	}
	 ...
	 */
	public function register_string($name, $string, $context = 'easyMultilingual', $multiline = false) {
		// backward compatibility with EasyMultilingual older than 1.1
		if (is_bool($context)) {
			$multiline = $context;
			$context = 'easyMultilingual';
		}
	...
	static public function list_metas_to_sync() {
		return array(
			'taxonomies'        => __('Taxonomies', 'easyMultilingual'),
			'post_meta'         => __('Custom fields', 'easyMultilingual'),
			'comment_status'    => __('Comment status', 'easyMultilingual'),
			'ping_status'       => __('Ping status', 'easyMultilingual'),
			'sticky_posts'      => __('Sticky posts', 'easyMultilingual'),
			'post_date'         => __('Published date', 'easyMultilingual'),
			'post_format'       => __('Post format', 'easyMultilingual'),
			'post_parent'       => __('Page parent', 'easyMultilingual'),
			'_wp_page_template' => __('Page template', 'easyMultilingual'),
			'menu_order'        => __('Page order', 'easyMultilingual'),
			'_thumbnail_id'     => __('Featured image', 'easyMultilingual'),
		);
	}
