...
<div id="col-container">
	...
	<div id="col-left">
		<div class="col-wrap">
			...
			<div class="form-wrap">
				<h3><?php echo $action=='edit' ? __('Edit language', 'easyMultilingual') :	__('Add new language', 'easyMultilingual'); ?></h3><?php
				...
				<div class="form-field">
					<label for="lang_list"><?php _e('Choose a language', 'easyMultilingual');?></label>
					<select name="lang_list" id="lang_list">
						<option value=""></option><?php
						include(EML_ADMIN_INC.'/languages.php');
					...
					<p><?php _e('You can choose a language in the list or directly edit it below.', 'easyMultilingual');?></p>
				</div>

				<div class="form-field form-required">
					<label for="lang_name"><?php _e('Full name', 'easyMultilingual');?></label>
					<input name="name" id="lang_name" type="text" value="<?php if ($action=='edit') echo esc_attr($edit_lang->name);?>" size="40" aria-required="true" />
					<p><?php _e('The name is how it is displayed on your site (for example: English).', 'easyMultilingual');?></p>
				</div>

				<div class="form-field form-required">
					<label for="lang_locale"><?php _e('Locale', 'easyMultilingual');?></label><?php
					printf(
						'<input name="locale" id="lang_locale" type="text" value="%s" size="40" aria-required="true" />',
						$action=='edit' ? esc_attr($edit_lang->locale) : ''
					);?>
					<p><?php _e('Wordpress Locale for the language (for example: en_US). You will need to install the .mo file for this language.', 'easyMultilingual');?></p>
				</div>

				<div class="form-field">
					<label for="lang_slug"><?php _e('Language code', 'easyMultilingual');?></label>
					<input name="slug" id="lang_slug" type="text" value="<?php if ($action=='edit') echo esc_attr($edit_lang->slug);?>" size="40"/>
					<p><?php _e('Language code - preferably 2-letters ISO 639-1  (for example: en)', 'easyMultilingual');?></p>
				</div>

				<div class="form-field"><fieldset>
					<legend><?php _e('Text direction', 'easyMultilingual');?></legend><?php
					printf(
						'<label><input name="rtl" type="radio" value="0" %s /> %s</label>',
						$action == 'edit' && $edit_lang->is_rtl ? '' : 'checked="checked"',
						__('left to right', 'easyMultilingual')
					);
					printf(
						'<label><input name="rtl" type="radio" value="1" %s /> %s</label>',
						$action == 'edit' && $edit_lang->is_rtl ? 'checked="checked"' : '',
						__('right to left', 'easyMultilingual')
					);?>
					<p><?php _e('Choose the text direction for the language', 'easyMultilingual');?></p>
				</fieldset></div>

				<div class="form-field">
					<label for="lang_order"><?php _e('Order', 'easyMultilingual');?></label>
					<input name="term_group" id="lang_order" type="text" value="<?php if ($action=='edit') echo esc_attr($edit_lang->term_group);?>" />
					<p><?php _e('Position of the language in the language switcher', 'easyMultilingual');?></p>
				</div>

				<?php submit_button( $action == 'edit' ? __('Update') : __('Add new language', 'easyMultilingual'), 'button'); // since WP 3.1 ?>
...
<div class="form-wrap">
	<form id="string-translation" method="post" action="admin.php?page=mlang&amp;tab=strings&amp;noheader=true">
		...
		printf('<br /><label><input name="clean" type="checkbox" value="1" /> %s</label>', __('Clean strings translation database', 'easyMultilingual')); ?>
		<p><?php _e('Use this to remove unused strings from database, for example after a plugin has been uninstalled.', 'easyMultilingual');?></p><?php
	...
	<table class="form-table">
		<tr>
			<th <?php echo $content_with_no_languages ? 'rowspan=2' : ''; ?>>
				<label for='default_lang'><?php _e('Default language', 'easyMultilingual');?></label>
			</th>
			<td><?php
				...
				<td>
					<label style="color: red"><?php
						printf(
							...
							__('There are posts, pages, categories or tags without language set. Do you want to set them all to default language ?', 'easyMultilingual')
						);?>
					</label>
				</td>
			</tr><?php
			...
			<td id="pll-hide-default" <?php echo 3 > $this->options['force_lang'] ? '' : 'style="display: none;"'; ?>><fieldset>
				<label><?php
					printf(
						...
						__('Hide URL language information for default language', 'easyMultilingual')
					);?>
				</label>
			</fieldset></td>
		</tr><?php
				...
				<td id="pll-rewrite" <?php echo 2 > $this->options['force_lang'] ? '' : 'style="display: none;"'; ?>><fieldset>
					<label><?php
						printf(
							...
							__('Remove /language/ in pretty permalinks', 'easyMultilingual')
						);?>
					</label>
					<p class="description"><?php echo __('Example:', 'easyMultilingual') . ' <code>'.esc_html(home_url('en/')).'</code>';?></p>
					<label><?php
						printf(
							'<input name="rewrite" type="radio" value="0" %s %s/> %s',
							$using_permalinks ? '' : 'disabled="disabled"',
							$this->options['rewrite'] ? '' : 'checked="checked"',
							 __('Keep /language/ in pretty permalinks', 'easyMultilingual')
						);?>
					</label>
					<p class="description"><?php echo __('Example:', 'easyMultilingual') . ' <code>'.esc_html(home_url('language/en/')).'</code>';?></p>
				</fieldset></td>
			</tr><?php
		}
				...
				<td><fieldset>
					<label><?php
						printf(
							...
							__('The front page url contains the language code instead of the page name or page id', 'easyMultilingual')
						);?>
					</label>
					<p class="description"><?php
						...
						printf(
							__('Example: %s instead of %s', 'easyMultilingual'),
			...
			<td>
				<label><?php
					printf(
						...
						__('When the front page is visited, set the language according to the browser preference', 'easyMultilingual')
					);?>
				</label>
			</td>
		</tr>
		...
		<tr>
			<th scope="row"><?php _e('Media', 'easyMultilingual') ?></th>
			<td>
				<label><?php
					printf(
						'<input name="media_support" type="checkbox" value="1" %s /> %s',
						$this->options['media_support'] ? 'checked="checked"' :'',
						__('Activate languages and translations for media', 'easyMultilingual')
					);?>
				</label>
			</td>
		</tr><?php
		...
		if (!empty($post_types)) {?>
			<tr>
				...
				<td>
					...
					<p class="description"><?php _e('Activate languages and translations for custom post types.', 'easyMultilingual');?></p>
				</td>
			</tr><?php
		}
		...
		if (!empty($taxonomies)) {?>
			<tr>
				...
				<td>
					...
					<p class="description"><?php _e('Activate languages and translations for custom taxonomies.', 'easyMultilingual');?></p>
				</td>
			</tr><?php
		}?>
		...
		<tr>
			...
			<td>
				...
				<p class="description"><?php _e('The synchronization options allow to maintain exact same values (or translations in the case of taxonomies and page parent) of meta content between the translations of a post or page.', 'easyMultilingual');?></p>
			</td>
		</tr>
