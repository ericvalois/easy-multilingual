 ...
 * accessible in $easyMultilingual global object
 *
 * properties:
 * options          => inherited, reference to EasyMultilingual options array
 * model            => inherited, reference to EML_Model object
 * links_model      => inherited, reference to EML_Links_Model object
 * settings_page    => optional, reference ot EML_Settings object
 * links            => reference to EML_Links object
 ...
 * filters          => reference to EML_Filters object
 * filters_columns  => reference to EML_Admin_Filters_Columns object
 * filters_post     => reference to EML_Admin_Filters_Post object
 * filters_term     => reference to EML_Admin_filters_Term object
 * nav_menu         => reference to EML_Admin_Nav_Menu object
 * sync             => reference to EML_Admin_Sync object
 * filters_media    => optional, reference to EML_Admin_Filters_Media object
 *
 * @since 1.2
 */
class EML_Admin extends EML_Base {
	...
	/*
	 * loads the easyMultilingual text domain
	...
	public function __construct(&$links_model) {
		...
		load_plugin_textdomain('easyMultilingual', false, basename(EASYMULTILINGUAL_DIR).'/languages');
		...
		add_filter('plugin_action_links_' . EASYMULTILINGUAL_BASENAME, array(&$this, 'plugin_action_links'));
		add_action('in_plugin_update_message-' . EASYMULTILINGUAL_BASENAME, array(&$this, 'plugin_update_message'), 10, 2);
	}
	...
	public function init() {
		if (EML_SETTINGS)
			$this->settings_page = new EML_Settings($this);

		if (!$this->model->get_languages_list())
			return;

		$this->links = new EML_Links($this);
	...
	public function add_menus() {
		add_submenu_page('options-general.php', $title = __('Languages', 'easyMultilingual'), $title, 'manage_options', 'mlang',
			EML_SETTINGS ? array($this->settings_page, 'languages_page') : create_function('', ''));
	}
	...
	public function admin_enqueue_scripts() {
		...
		if (version_compare($GLOBALS['wp_version'], '3.8alpha' , '>='))
			wp_enqueue_style('easyMultilingual_admin_mobi', EASYMULTILINGUAL_URL .'/css/admin-mobi'.$suffix.'.css', array(), EASYMULTILINGUAL_VERSION);
	}
...
<script type="text/javascript">
	...
	public function plugin_action_links($links) {
		array_unshift($links, '<a href="admin.php?page=mlang">' . __('Settings', 'easyMultilingual') . '</a>');
		return $links;
	}
