 ...
 * @since 1.2
 */
abstract class EML_Choose_Lang {
	...
	public function __construct(&$easyMultilingual) {
		$this->links = &$easyMultilingual->links;
		...
		if (EML_AJAX_ON_FRONT || false === stripos($_SERVER['SCRIPT_FILENAME'], 'index.php'))
	 ...
	 * possibility to set EML_COOKIE to false will disable cookie although it will break some functionalities
	 *
	 * @since 1.5
	 */
	protected function maybe_setcookie() {
		// check headers have not been sent to avoid ugly error
		if (!headers_sent() && EML_COOKIE !== false && (!isset($_COOKIE[EML_COOKIE]) || $_COOKIE[EML_COOKIE] != $this->curlang->slug))
			setcookie(EML_COOKIE, $this->curlang->slug, time() + 31536000 /* 1 year */, COOKIEPATH, parse_url(get_option('home'), PHP_URL_HOST));
	}
	...
	public function get_preferred_language() {
		...
		if (isset($_COOKIE[EML_COOKIE]))
			return $this->model->get_language($_COOKIE[EML_COOKIE]);
	...
	protected function home_language() {
		// test referer in case EML_COOKIE is set to false
	...
	public function home_requested() {
		...
		// FIXME why this happens? http://wordpress.org/support/topic/easyMultilingual-crashes-1
	...
	public function parse_main_query($query) {
		...
		// as well as when searching an empty string: http://wordpress.org/support/topic/plugin-easyMultilingual-easyMultilingual-breaks-search-in-spun-theme
