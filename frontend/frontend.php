...
/*
 * frontend controller
 * accessible as $easyMultilingual global object
 *
 * properties:
 ...
 * choose_lang      => reference to EML_Choose_lang object
 * curlang          => current language
 * filters          => reference to EML_Filters object
 * filters_search   => reference to EML_Frontend_Filters_Search object
 * nav_menu         => reference to EML_Frontend_Nav_Menu object
 * auto_translate   => optional, reference to EML_Auto_Translate object
 *
 * @since 1.2
 */
class EML_Frontend extends EML_Base {
	public $curlang;
	...
	public function __construct(&$links_model) {
		...
		if (!defined('EML_AUTO_TRANSLATE') || EML_AUTO_TRANSLATE)
	...
	public function init() {
		$this->links = new EML_Frontend_Links($this);

		$c = array('Content', 'Url', 'Url', 'Domain');
		$class = 'EML_Choose_Lang_' . $c[$this->options['force_lang'] * (get_option('permalink_structure') ? 1 : 0 )];
	...
	public function pll_language_defined($slug, $curlang) {
		...
		// filters
		$this->filters = new EML_Frontend_Filters($this);
		$this->filters_search = new EML_Frontend_Filters_Search($this);

		// nav menu
		$this->nav_menu = new EML_Frontend_Nav_Menu($this);
	}
	...
	public function auto_translate() {
		$this->auto_translate = new EML_Frontend_Auto_Translate($this);
	}
