...
(function($){
	...
	tagBox.suggest = function() {
		...
		$('input.newtag', ajaxtag).unbind().blur(function() {
			...
			$(this).suggest( ajaxurl + '?action=easyMultilingual-ajax-tag-search&lang=' + lang + '&tax=' + tax, { delay: 500, minchars: 2, multiple: true, multipleSep: "," } );
		});
	}
...
jQuery(document).ready(function($) {
	...
	function init_translations() {
		$('.tr_lang').each(function(){
			...
			var td = $(this).parent().siblings('.eml-edit-column');

			$(this).autocomplete({
				minLength: 0,

				source: ajaxurl + '?action=eml_posts_not_translated&post_language=' + $('#post_lang_choice').val() +
