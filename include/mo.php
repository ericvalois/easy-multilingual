 ...
 * @since 1.2
 */
class EML_MO extends MO {

	/*
	 * registers the easyMultilingual_mo custom post type, only at first object creation
	 *
	 * @since 1.2
	 */
	public function __construct() {
		if (!post_type_exists('easyMultilingual_mo'))
			register_post_type('easyMultilingual_mo', array('rewrite' => false, 'query_var' => false, '_pll' => true));
	}

	/*
	 * writes a EML_MO object into a custom post
	 *
	 * @since 1.2
	 *
	...
	public function export_to_db($lang) {
		...
		if (empty($post))
				$GLOBALS['easyMultilingual']->model->clean_languages_cache(); // to set mo_id

		$post['post_title'] = 'easyMultilingual_mo_' . $lang->term_id;
		...
		$post['post_type'] = 'easyMultilingual_mo';
		wp_insert_post($post);
	}

	/*
	 * reads a EML_MO object from a custom post
	 *
	 * @since 1.2
	 *
	...
	public static function get_id($lang) {
		global $wpdb;
		return $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type= %s", 'easyMultilingual_mo_' . $lang->term_id, 'easyMultilingual_mo'));
	}
}
