 ...
 * in 'setup_theme' by EasyMultilingual (based on user info) and 'plugins_loaded' by WPML (based on cookie)
 * moreover I believe that WPML can set ICL_LANGUAGE_CODE to 'all' which I dont want to do with EasyMultilingual
 *
 * @since 0.9.5
 */
function eml_define_wpml_constants() {
	...
	if (!empty($easyMultilingual->curlang)) {
		if(!defined('ICL_LANGUAGE_CODE'))
			define('ICL_LANGUAGE_CODE', $easyMultilingual->curlang->slug);

		if(!defined('ICL_LANGUAGE_NAME'))
			define('ICL_LANGUAGE_NAME', $easyMultilingual->curlang->name);
	}
 ...
 * translated_name  => empty, does not exist in EasyMultilingual
...
if (!function_exists('icl_get_languages')) {
	function icl_get_languages($args = '') {
		global $easyMultilingual;
		if (empty($easyMultilingual) || empty($easyMultilingual->curlang))
			return array();
		...
		foreach ($easyMultilingual->model->get_languages_list(array('hide_empty' => true, 'orderby' => $orderby, 'order' => $order)) as $lang) {
			$url = $easyMultilingual->links->get_translation_url($lang);
			...
			$arr[] = array(
				'id'               => $lang->term_id,
				'active'           => isset($easyMultilingual->curlang->slug) && $easyMultilingual->curlang->slug == $lang->slug ? 1 : 0,
				'native_name'      => $lang->name,
				'missing'          => empty($url) ? 1 : 0,
				'translated_name'  => '', // does not exist in EasyMultilingual
				'language_code'    => $lang->slug,
				'country_flag_url' => $lang->flag_url,
				'url'              => $url ? $url :
					(empty($link_empty_to) ? $easyMultilingual->links->get_home_url($lang) :
 ...
 * @param bool $return_original_if_missing optional, true if EasyMultilingual should return the original id if the translation is missing, defaults to false
 ...
 * @since 0.9.3
 *
 * @param string $context the group in which the string is registered, defaults to 'easyMultilingual'
 ...
 * @since 0.9.3
 *
 * @param string $context not used by EasyMultilingual
 * @param string $name not used by EasyMultilingual
 ...
 * @param string $name a unique name for the string
 * @param string $string the string to register
 * @param bool $bool not used by EasyMultilingual
 * @return string the translated string in the current language
 */
 ...
 * this is probably linked to the fact that WPML has always an original post in the default language and not EasyMultilingual :)
 *
 * @since 1.1.2
 *
 * @return array
...
if (!function_exists('wpml_get_copied_fields_for_post_edit')) {
	function wpml_get_copied_fields_for_post_edit() {
		...
		// don't know what WPML does but EasyMultilingual does copy all public meta keys by default
...
class EML_WPML_Compat {
	...
	public function register_string($context, $name, $string) {
		if (empty($this->strings))
			$this->strings = get_option('easyMultilingual_wpml_strings', array());
		...
		if (!in_array($to_register, $this->strings) && $to_register['string']) {
			...
			update_option('easyMultilingual_wpml_strings', $this->strings);
		}
	}
	...
	public function get_strings($strings) {
		if (empty($this->strings))
			$this->strings = get_option('easyMultilingual_wpml_strings');
...
class EML_WPML_Config {
	...
	public function init() {
		...
		// plugins
		// don't forget sitewide active plugins thanks to Reactorshop http://wordpress.org/support/topic/easyMultilingual-and-yoast-seo-plugin/page/2?replies=38#post-4801829
		...
		foreach ($plugins as $plugin) {
			if (file_exists($file = dirname(EASYMULTILINGUAL_DIR).'/'.dirname($plugin).'/wpml-config.xml'))
				$this->xml_parse(file_get_contents($file), dirname($plugin));
		}

		// custom
		if (file_exists($file = EML_LOCAL_DIR.'/wpml-config.xml'))
	 ...
	 * @since 1.0
	 *
	 * @param array $types list of post type names for which EasyMultilingual manages language and translations
	 * @param bool $hide true when displaying the list in EasyMultilingual settings
	 * @return array list of post type names for which EasyMultilingual manages language and translations
	 */
	 ...
	 * @since 1.0
	 *
	 * @param array $taxonomies list of taxonomy names for which EasyMultilingual manages language and translations
	 * @param bool $hide true when displaying the list in EasyMultilingual settings
	 * @return array list of taxonomy names for which EasyMultilingual manages language and translations
	 */
