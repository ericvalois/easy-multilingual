...
function eml_the_languages($args = '') {
	...
	if ($easyMultilingual instanceof EML_Frontend && !empty($easyMultilingual->links)) {
		...
		return $switcher->the_languages($easyMultilingual->links, $args);
	}
	return '';
}
...
function eml_translate_string($string, $lang) {
	...
	if (empty($mo[$lang])) {
		$mo[$lang] = new EML_MO();
		$mo[$lang]->import_from_db($GLOBALS['easyMultilingual']->model->get_language($lang));	
	}
...
/*
 * returns true if EasyMultilingual manages languages and translations for this taxonomy
 *
 * @since 1.0.1
 *
