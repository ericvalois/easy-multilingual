 ...
 * tl_count            => number of terms in that language (not used by EasyMultilingual)
...
class EML_Language {
	...
	public function __construct($language, $term_language) {
		...
		$this->description = &$this->locale; // backward compatibility with EasyMultilingual < 1.2

		$this->mo_id = EML_MO::get_id($this);
	}
	...
	public function set_flag() {
		if (file_exists(EASYMULTILINGUAL_DIR.($file = '/flags/'.$this->locale.'.png')))
			$url = EASYMULTILINGUAL_URL.$file;

		// overwrite with custom flags
		// never use custom flags on admin side
		if (!EML_ADMIN && ( file_exists(EML_LOCAL_DIR.($file = '/'.$this->locale.'.png')) || file_exists(EML_LOCAL_DIR.($file = '/'.$this->locale.'.jpg')) ))
			$url = EML_LOCAL_URL.$file;
	...
	public function set_home_url() {
		...
		$this->search_url = $GLOBALS['easyMultilingual']->links_model->home_url($this);
