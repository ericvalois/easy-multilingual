...
class EML_Model {
	 ...
	 * @since 1.2
	 *
	 * @param array $options EasyMultilingual options
	 */
	 ...
	 * @param string $taxonomy EasyMultilingual taxonomy depending if we are looking for a post (or term) language (or translation)
	 ...
	 * caches the list in a db transient (except flags), unless EML_CACHE_LANGUAGES is set to false
	 ...
	 * fields     => return only that field if set (see EML_Language for a list of fields)
	 *
	 * @since 0.1
	 *
	 * @param array $args
	 * @return array|string|int list of EML_Language objects or EML_Language object properties
	 */
			...
			if ((defined('EML_CACHE_LANGUAGES') && !EML_CACHE_LANGUAGES) || false === ($languages = get_transient('pll_languages_list'))) {
				...
				if (!empty($languages) && !empty($term_languages)) {
					...
					foreach ($languages as $k => $v) {
						$languages[$k] = new EML_Language($v, $term_languages[$v->name]);
					}
				}
				else {
	 ...
	 * languages objects are not cached in db if EML_CACHE_LANGUAGES is set to false
	 * home urls are not cached in db if EML_CACHE_HOME_URL is set to false
	 *
	 * @since 1.4
	 */
	 ...
	 * @return object|bool EML_Language object, false if no language found
	 */
	public function get_language($value) {
		static $language;

		if (is_object($value))
			return $this->get_language($value->term_id); // will force cast to EML_Language
	 ...
	 *
	 * @param int $post_id post id
	 * @return bool|object EML_Language object, false if no language is associated to that post
	 */
	 ...
	 * @return bool|object EML_Language object, false if no language is associated to that term
	 */
	 ...
	 * @since 1.2
	 *
	 * @param object|array|string $lang a EML_Language object or a comma separated list of languag slug or an array of language slugs
	 * @param string $type either 'post' or 'term'
	 ...
	 * @param object $lang EML_Language object
	...
	public function register_taxonomy() {
		...
		register_taxonomy('language', $this->get_translated_post_types(), array(
			'labels' => array(
				'name' => __('Languages', 'easyMultilingual'),
				'singular_name' => __('Language', 'easyMultilingual'),
				'all_items' => __('All languages', 'easyMultilingual'),
			),
			'public' => false, // avoid displaying the 'like post tags text box' in the quick edit
			'query_var' => 'lang',
			'_pll' => true // easyMultilingual taxonomy
		));
	}
	 ...
	 * @return array post type names for which EasyMultilingual manages languages and translations
	 */
	...
	/*
	 * returns true if EasyMultilingual manages languages and translations for this post type
	 *
	 * @since 1.2
	 *
	 ...
	 * @return array array of registered taxonomy names for which EasyMultilingual manages languages and translations
	 */
	 ...
	 * @since 1.4
	 *
	 * @param object $lang a EML_Language object
	...
	public function get_links_model() {
		...
		$class = get_option('permalink_structure') ? 'EML_Links_' .$c[$this->options['force_lang']] : 'EML_Links_Default';
