 ...
 * @since 0.1
 */
class EML_Widget_Languages extends WP_Widget {
	...
	function __construct() {
		parent::__construct('easyMultilingual', __('Language Switcher', 'easyMultilingual'), array( 'description' => __( 'Displays a language switcher', 'easyMultilingual')));
	}
	...
	function widget($args, $instance) {
		...
		if ($dropdown) {
			foreach ($easyMultilingual->model->get_languages_list() as $language) {
				$url = $force_home || ($url = $easyMultilingual->links->get_translation_url($language)) == null ? $easyMultilingual->links->get_home_url($language) : $url;
	...
	function update( $new_instance, $old_instance ) {
		...
		foreach (array_keys(EML_Switcher::get_switcher_options('widget')) as $key)
	...
	function form($instance) {
		// default values
		$instance = wp_parse_args( (array)$instance, array_merge(array('title' => ''), EML_Switcher::get_switcher_options('widget', 'default')) );

		// title
		$title = sprintf(
			'<p><label for="%1$s">%2$s</label><input class="widefat" id="%1$s" name="%3$s" type="text" value="%4$s" /></p>',
			$this->get_field_id('title'),
			__('Title:', 'easyMultilingual'),
			$this->get_field_name('title'),
			esc_attr($instance['title'])
		);

		$fields = '';
		foreach (EML_Switcher::get_switcher_options('widget') as $key => $str)
			$fields .= sprintf(
