...
class EML_Plugins_Compat {
	...
	public function __construct() {
		...
		// YARPP
		// just makes YARPP aware of the language taxonomy (after EasyMultilingual registered it)
	...
	/*
	 * WordPress Importer
	 * loads our child class EML_WP_Import instead of WP_Import
	 *
	 * @since 1.2
	 */
	function wordpress_importer_init() {
		...
		$GLOBALS['wp_import'] = new EML_WP_Import();
	...
	public function wpseo_translate_options() {
		if (defined('WPSEO_VERSION') && !EML_ADMIN && did_action('wp_loaded')) {
			global $wpseo_front;
	...
	public function twenty_fourteen_featured_content_ids($featured_ids) {
		...
		$tags = $GLOBALS['easyMultilingual']->model->get_translations('term' ,$term->term_id);
		$ids = array();
