 ...
 * @since 0.5
 */
class EML_Widget_Calendar extends WP_Widget_Calendar {
	...
	function widget( $args, $instance ) {
		...
		empty($GLOBALS['easyMultilingual']->curlang) ? get_calendar() : self::get_calendar(); #modified#
		echo '</div>';
		echo $after_widget;
	}
	...
	static function get_calendar($initial = true, $echo = true) {
		global $wpdb, $m, $monthnum, $year, $wp_locale, $posts, $easyMultilingual; #modified#

		$join_clause = $easyMultilingual->model->join_clause('post'); #added#
		$where_clause = $easyMultilingual->model->where_clause($easyMultilingual->curlang, 'post'); #added#

		$cache = array();
		$key = md5( $easyMultilingual->curlang->slug . $m . $monthnum . $year ); #modified#
