...
/*
 * manages EasyMultilingual upgrades
 *
 * @since 1.2
 */
class EML_Upgrade {
	...
	/*
	 * upgrades if possible otherwise returns false to stop EasyMultilingual loading
	 *
	 * @since 1.2
	 *
	...
	public function admin_notices() {
		...
		printf(
			...
			__('EasyMultilingual has been deactivated because you upgraded from a too old version.', 'easyMultilingual'),
			sprintf(
				__('Please upgrade first to %s before ugrading to %s.', 'easyMultilingual'),
				'<strong>0.9.8</strong>',
				EASYMULTILINGUAL_VERSION
			)
		);
	}
	...
	protected function upgrade_0_9() {
		$this->options['sync'] = defined('EML_SYNC') && !EML_SYNC ? 0 : 1; // the option replaces EML_SYNC in 0.9
	}
	...
	protected function upgrade_1_0() {
		// the option replaces EML_MEDIA_SUPPORT in 1.0
		$this->options['media_support'] = defined('EML_MEDIA_SUPPORT') && !EML_MEDIA_SUPPORT ? 0 : 1;

		// split the synchronization options in 1.0
		$this->options['sync'] = empty($this->options['sync']) ? array() : array_keys(EML_Settings::list_metas_to_sync());
	...
	protected function upgrade_1_1() {
		...
		$strings = get_option('easyMultilingual_wpml_strings');
		if ($strings) {
			foreach ($strings as $key => $string)
				$strings[$key]['icl'] = 1;
			update_option('easyMultilingual_wpml_strings', $strings);
		}

		// move easyMultilingual_widgets options
		if ($widgets = get_option('easyMultilingual_widgets')) {
			$this->options['widgets'] = $widgets;
			delete_option('easyMultilingual_widgets');
		}
	}
	...
	protected function upgrade_1_2_1() {
		...
		foreach(get_terms('language', array('hide_empty' => 0)) as $lang) {
			if ($strings = get_option('easyMultilingual_mo'.$lang->term_id)) {
				$mo = new EML_MO();
	...
	public function upgrade_1_2_3() {
		...
		if (version_compare($this->options['version'], '1.1', '<')) {
			if ($menu_lang = get_option('easyMultilingual_nav_menus')) {
				...
				delete_option('easyMultilingual_nav_menus');
			}
